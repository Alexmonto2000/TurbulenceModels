...
#include "kOmegaSSTML_SpaRTA60120.H"
...
void kOmegaSSTML_SpaRTA60120<BasicTurbulenceModel>::correctNut(const volScalarField& S2)
{
...
template<class BasicTurbulenceModel>
void kOmegaSSTML_SpaRTA60120<BasicTurbulenceModel>::correctNut()
{
...
template<class BasicTurbulenceModel>
kOmegaSSTML_SpaRTA60120<BasicTurbulenceModel>::kOmegaSSTML_SpaRTA60120
(
...
template<class BasicTurbulenceModel>
tmp<fvVectorMatrix> kOmegaSSTML_SpaRTA60120<BasicTurbulenceModel>::divDevReff
(
    volVectorField& U
) const
{
    Info << "In: kOmegaSSTML_SpaRTA60120::divDevReff()" << endl;
    return
    (
...
kOmegaSSTML_SpaRTA60120<BasicTurbulenceModel>::devRhoReff() const
{
    Info << "In: kOmegaSSTML_SpaRTA60120::devRhoReff()" << endl;
...
kOmegaSSTML_SpaRTA60120<BasicTurbulenceModel>::divDevRhoReff
(
    volVectorField& U
) const
{
    Info << "In: kOmegaSSTML_SpaRTA60120::divDevRhoReff(U)" << endl;
    return
    (
...
{
    Info << "In: kOmegaSSTML_SpaRTA60120::divDevRhoReff(rho, U)" << endl;
    return
    (
...
template<class BasicTurbulenceModel>
void kOmegaSSTML_SpaRTA60120<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
    }
    Info << "In: kOmegaSSTML_SpaRTA60120.correct()" << endl;
    // Local references
...
/*******************************Model Addition**************************/
    ...
    kDeficit_ = max(classifier * ((-1.0303858 * (Ak2/0.6834722)*(G1_s/0.028767251) +
     12.026683 * (AlfAkS_s/0.50413743)*(G1_s/0.028767251) + 0.0042812005 * exp((q_Re/0.40824685))*(G1_s/0.028767251) +
      0.4009921 * pow(Ak2WS_s/0.050458403 ,2)*(G1_s/0.028767251) + -117.44846 * ((Alf2/38.046803) / (1 + pow(Alf2/38.046803 , 2)))*(G1_s/0.028767251) +
       0.70061418 * ((q_Re/0.40824685) / (1 + pow(q_Re/0.40824685, 2)))*(G1_s/0.028767251) +
        0.5625679 * sqrt(pow(AlfAkS_s/0.50413743 , 2))*(G1_s/0.028767251)) * 0.0059344529),
